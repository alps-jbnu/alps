...
__all__ = ('configure', 'get_publisher_html', 'score_result')
...
def configure(publisher_key, scoring_key, ws_host='ws.areyouahuman.com'):
    """
    Sets the publisher key and scoring key needed to make subsequent calls
    to the areyouahuman API. Call this function once
...
def check_configuration():
    if not config:
        raise Exception(
            'You must call ayah.configure() before using this API.'
        )


def get_publisher_html():
    """
    Gets the HTML markup that displays the PlayThru content to the alleged
    human. When the alleged human finishes the PlayThru challenge, pass the
    value of the hidden input field with id='session_secret'
    to score_result().
    """
